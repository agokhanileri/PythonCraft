"""Content: Soft level practice questions."""

# Tags: Basic, Math, LL, BT, Graph, Bits, Hash, Stacks, I/O, Algo
# - where Algo = Search, Sort, Dynamic, Greedy, D&C, Graph

import math

import numpy as np
from aux_scripts import binary_search, quick_sort
...
# =================================================================================================
# Containers: Print a list in reverse order using loops.
arr1 = [1, 2, 3]
...
# -------------------------------------------------------------------------------------------------
# Containers: Print all the common elements of two lists.
arr1 = [1, 2, 3, 4]
arr2 = [3, 4, 5, 6]
...
# -------------------------------------------------------------------------------------------------
# Sort: Find 3, and then sort the list by using algos imported.
arr = [1, 5, 3, 2, -4, -3]

# Sol:
a = binary_search(arr, 3)
b = quick_sort(arr)
print(a, b)


# -------------------------------------------------------------------------------------------------
# Containers: Given a list, print the # of numbers in the given range.
# Given: Sorted? --> no, Repetative? --> yes, Range is sorted? --> yes
...
# -------------------------------------------------------------------------------------------------
# Q: Append at the beginning of arr
arr1 = [10, 12, 13, 17]
arr2 = [40, 50]
...
for i, j in zip(arr1, arr2, strict=True):
...
# Sol: use Zip
zipped = zip(arr2, arr1, strict=True)  # zipped pair
...
# -----------------------------------
# Containers: Union of two or more Lists
...
# -----------------------------------
# Interactive: Print a triangle (use same n)
# Sol:
m = math.floor(n / 2)
for i in range(1, n + 1):  # for each row
...
# -----------------------------------
# Interactive: Print a number pattern (use same n)
# Sol:
str = ""
for i in range(1, n + 1):  # for each row
...
# -----------------------------------
# Interactive: Print out the grade-school multiplication table up to 12x12 (no tabs)
n = 12
...
# -----------------------------------
# Containers: Convert a list of Tuples into Dictionary
# Given:
tups = [("A", 1), ("B", 2), ("C", 3)]
n = len(tups)
...
# -----------------------------------
# Containers: Reverse an array in groups of given size
# Q:
# Given:
arr = [1, 2, 3, 4, 5, 6, 7, 8]
n = len(arr)
...
# Sort: Sort a list according to the second element in sublist
...
for j in range(256):
    if hash_chars[j] % 2 == 1:  # odd length --> need 1 single, (n-1)/2 twin chars
        ch_odd += 1
    # else:  # .. % 2 == 0 --> even
if (n % 2 == 0) and (ch_odd == 0) or (n % 2 == 1) and (ch_odd == 1):  # even length + no odds
    out = True
else:
    out = False
out
