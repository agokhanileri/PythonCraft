...
# Methods: binary_search, quick_sort   - to use methods: from ScriptPull import *


# =================================================================================================
# Compare
def compare(num1, num2):
    """Comparison like math.cmp(a,b) that is N/A for Python 3."""
...
def binary_search(arr, val):
    """Binary search of a given value in a given array."""
    n = len(arr)
    i = int(round(n / 2, 0))
    cnt = 1
    while i < n:
...
def partition(arr, left, right):
    """Divides Binary search of a given value in a given array."""
    i = left - 1  # index to place the valsfound, start at -1
    # j = L                                  # index to compare to pivot
    for j in range(left, right):  # pivot is the R
        # print(L, R, i, j, arr)
        if arr[j] < arr[right]:  # if smaller value found
...
def quick_sort(arr, left, right):
    """Recursion to divide, real issue is to merge."""
    if left < right:
